...
public class PetStoreSteps {
	...
	public void i_am_able_to_perform_post_request_to_create_a_pet_with_valid_request_body() {
		...
		response = given().contentType(ContentType.JSON).accept(ContentType.JSON).body(responseBody).when().post(URL);
		...
		// Assert.assertEquals(id, 1121345);
		Assert.assertEquals(response.path("name"), "Dog");
		Assert.assertEquals(response.path("status"), "avaliable");

	}
	
	// post to add a pet scenario #Starts

	@Given("I have the POST request URL with request body")
	public void i_have_the_post_request_url_with_request_body() {
	...
	public void i_perform_post_request_to_url_with_request_body() {
		response = given().contentType(ContentType.JSON).accept(ContentType.JSON).body(requestBody).when().post(requestURL);
	}
	...
	public void response_body_match_the_request_body() {
		...
		.and().assertThat().body("id", equalTo(1234234))
		.and().assertThat().body("name", equalTo("doggie"))
		.and().assertThat().body("status", equalTo("available"))
		.and().assertThat().body("category.name", equalTo("Sumo"))
		.and().assertThat().body("tags[0].name",equalTo("shitzu"));
	}
	
	// invalid post request body scenario #Starts

	@Given("I have the POST request URL with invalid request body")
	public void i_have_the_post_request_url_with_invalid_request_body() {
		requestURL = "https://petstore.swagger.io/v2/pet";
		requestBody = new File("./src/test/resources/jsonFiles/invalidRequestBody.json");
	}
	
	@Then("Response status code should be {int} invalid imput")
	public void response_status_code_should_be_invalid_imput(Integer expectedStatusCode) {
		response.then().assertThat().statusCode(expectedStatusCode);
	}
	...
	public void i_send_a_get_request_by_to_invalid_endpoint(String expectedStatus) {
	...
	public void i_perform_update_request_to_update_name_to_existing_pet() {
	...
	@Then("Id is {int} and Name is {string}")
	public void id_is_and_name_is(Integer petID, String petName) {
		response.then().assertThat().body("id", equalTo(petID))
		.and().assertThat().body("name", equalTo(petName));
	}
