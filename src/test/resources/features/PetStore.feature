Feature: As a user I am able to perform HTTP request
  ...
  @invalidPostRequest
  Scenario: As a user, I should notbe able to perform POST request to add new pet to store
    Given I have the POST request URL with invalid request body
    When I perform POST request to URL with request body
    Then Response status code should be 500 invalid imput
    And Content type is "application/json"
  ...
  @getByPetID
  Scenario: As a user, I should be able to perform GET request to find a pet
  ...
  Scenario: As a user, I am able to update a pet
    ...
    Then Response status code should be 200
    And Content type should be "application/json"
    And Id is 1234234 and Name is "Kuma"
